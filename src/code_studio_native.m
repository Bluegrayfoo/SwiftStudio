...
- (void)placePreviewContent {
  ...
  if (inlinePreview) {
    if (self.previewWindow.visible) [self.previewWindow orderOut:nil];
    ...
    if (self.previewContentView.superview != self.previewContainerView) {
      [self.previewContentView removeFromSuperview];
  ...
  self.previewContentView.autoresizingMask = NSViewWidthSizable | NSViewHeightSizable;
  if (self.previewContentView.superview != host) {
    [self.previewContentView removeFromSuperview];
    [host addSubview:self.previewContentView];
  }
  if (!self.previewWindow.visible) [self.previewWindow makeKeyAndOrderFront:nil];
}
